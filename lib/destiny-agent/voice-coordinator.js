...
export class VoiceResponseCoordinator {
  ...
  audioCleared(responseId, { initiator = "provider" } = {}) { return this.#dispatch({ type: "AUDIO_CLEARED", responseId, initiator }); }
  assistantItem(responseId, itemId) { return this.#dispatch({ type: "ASSISTANT_ITEM", responseId, itemId }); }
  historyTruncated(itemId) { return this.#dispatch({ type: "HISTORY_TRUNCATED", itemId }); }
  retireIfHistoryPending(responseId) { return this.#dispatch({ type: "HISTORY_SYNC_TIMEOUT", responseId }); }
  ...
  probeCancellation(requestToken) {
    return this.#dispatch({ type: "CANCELLATION_PROBE", requestToken });
  }
  ...
  #reduce(event, effects) {
    ...
    if (event.type === "CANCELLATION_PROBE") {
      if (!this.lease || !this.lease.interrupted || this.lease.requestToken !== event.requestToken) return false;
      this.lease.probeAttempts += 1;
      if (this.lease.probeAttempts > 1) {
        effects.push({ type: "retire_connection", reason: "cancellation_state_unproven", lease: cloneLease(this.lease) });
        return true;
      }
      if (!this.lease.generationTerminal && this.lease.responseId) effects.push({ type: "send_cancel", responseId: this.lease.responseId, probe: true, lease: cloneLease(this.lease) });
      if (!this.lease.playbackTerminal) effects.push({ type: "clear_audio", responseId: this.lease.responseId, probe: true, lease: cloneLease(this.lease) });
      effects.push({ type: "probe_sent", reason: "awaiting_terminal_acknowledgement", lease: cloneLease(this.lease) });
      return true;
    }
    ...
    if (event.type === "SPEECH_STARTED") {
      ...
      if (this.lease && !this.lease.playbackTerminal) {
        this.lease.inputDecisionPending = event.candidateId;
    ...
    if (event.type === "SPEECH_RESTORE") {
      ...
      if (this.lease) {
        this.lease.ducked = false;
        if (!event.candidateId || this.lease.inputDecisionPending === event.candidateId) this.lease.inputDecisionPending = null;
        if (this.lease.providerCleared && this.lease.playbackTerminal) this.lease.repairRequired = true;
      }
      this.candidate = null;
    ...
    if (event.type === "INTERRUPT") {
      ...
      this.lease.ducked = true;
      this.lease.inputDecisionPending = null;
    ...
    if (event.type === "ASSISTANT_ITEM") {
      if (!this.#matches(event.responseId) || !event.itemId) return false;
      this.lease.outputItemId = String(event.itemId);
      return true;
    }

    if (event.type === "HISTORY_TRUNCATED") {
      if (!this.lease?.historySyncPending) return false;
      if (this.lease.outputItemId && event.itemId && String(event.itemId) !== this.lease.outputItemId) return false;
      this.lease.historySyncPending = false;
      this.lease.historyTruncated = true;
      this.#releaseIfTerminal(effects);
      return true;
    }

    if (event.type === "HISTORY_SYNC_TIMEOUT") {
      if (!this.#matches(event.responseId) || !this.lease.historySyncPending) return false;
      effects.push({ type: "retire_connection", reason: "history_sync_unproven", lease: cloneLease(this.lease) });
      return true;
    }
    ...
    if (event.type === "AUDIO_CLEARED") {
      ...
      this.lease.playbackTerminal = true;
      this.lease.clearInitiator = event.initiator;
      if (event.initiator === "provider" && !this.lease.clearRequested) {
        this.lease.providerCleared = true;
        this.lease.historySyncPending = true;
      }
      this.#releaseIfTerminal(effects);
      return true;
    }
  ...
  #releaseIfTerminal(effects) {
    if (!this.lease?.generationTerminal || !this.lease?.playbackTerminal || this.lease?.inputDecisionPending || this.lease?.historySyncPending) return;
    const released = cloneLease(this.lease);
    const repairRequired = this.lease.repairRequired && !this.lease.interrupted;
    this.lease = null;
    this.candidate = null;
    effects.push({ type: "restore_audio", reason: "output_terminal" });
    effects.push({ type: "released", lease: released });
    if (repairRequired) {
      effects.push({ type: "repair_required", reason: "provider_clear_false_interruption", lease: released });
      return;
    }
