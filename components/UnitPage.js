  ...
  return <div className={styles.page}>
    ...
    <main>
      <section className={styles.propertyShowcase}>
        ...
        <div className={styles.previewMosaic} aria-label={`Photo preview for Pelican Beach Resort Unit ${unit.number}`}>
      ...
      <section className={styles.availability} id="availability"><div className={styles.sectionHead}><p className={styles.kicker}>Unit {unit.number} calendar</p><h2>See available and unavailable dates.</h2><p>This live multi-month calendar is specific to this condo. Use the checkout above to confirm your dates, guest count and complete total.</p></div><div className={styles.calendarShell}><div className="ownerrez-widget" data-propertyid={unit.propertyId} data-widget-type="Multiple Month Calendar" data-widgetid={calendarWidgetId}></div><noscript><a href={`${unit.ownerRezUrl}#availability`}>Open the live availability calendar for Unit {unit.number}</a></noscript></div></section>
      ...
      <section className={styles.gallerySection} id="photos"><div className={styles.sectionHead}><p className={styles.kicker}>Complete photo gallery</p><h2>See every room, view and resort amenity.</h2><p>The complete property photo collection is included. Select any image to enlarge it and use the arrow keys or controls to continue.</p></div><div className={styles.gallery}>{photos.slice(0, 12).map((src, index) => <button key={src} type="button" onClick={() => setLightbox(index)} aria-label={`Enlarge ${photoDescription(unit, index)}`}><img src={src} alt={photoDescription(unit, index)} loading={index < 3 ? "eager" : "lazy"} width="1200" height="800" /></button>)}</div><button className={styles.allPhotos} type="button" onClick={() => setLightbox(0)}>View all {photos.length} photos</button></section>
