...
import { searchPublishedKnowledge } from "./published-knowledge.js";
...
export function createServices({ fetchImpl = globalThis.fetch, env = process.env, now = () => new Date(), logger = console } = {}) {
  ...
  async function fetchPublishedKnowledge(query, topics = [], limit = 8) {
    return searchPublishedKnowledge({ query, topics, limit, fetchImpl, env });
  }
  ...
  async function sendOwnerChatInvite({ sessionId, guestMessage, inviteToken }) {
    try {
      ...
      // Keep the owner page on the same deployment as the guest conversation.
      // This matters for Preview testing, where production may run older API code.
      const deploymentHost = String(env.VERCEL_URL || "").replace(/^https?:\/\//, "").replace(/\/$/, "");
      const ownerChatBaseUrl = deploymentHost
        ? `https://${deploymentHost}`
        : "https://destin-concierge-new.vercel.app";
      const enterChatUrl = `${ownerChatBaseUrl}/ozan?s=${encodeURIComponent(sessionId)}&t=${encodeURIComponent(inviteToken)}`;
      const response = await fetchWithTimeout(fetchImpl, `https://discord.com/api/v10/channels/${channelId}/messages`, {
        method: "POST",
  ...
  async function fetchDestinWeather() {
    try {
      ...
      if (!res.ok) {
        logger.error("Google Weather HTTP error:", res.status);
        return { status: "unavailable", reason: `http_${res.status}`, forecast: [] };
      }
      const data = await res.json();
      const days = Array.isArray(data?.forecastDays) ? data.forecastDays : [];
      const forecast = days.flatMap(day => {
        // Google returns the civil date as displayDate. interval.startTime is
        // retained as a defensive fallback because the legacy TV endpoint uses it.
        const displayDate = day?.displayDate;
        const displayDateIso = Number.isInteger(Number(displayDate?.year))
          && Number.isInteger(Number(displayDate?.month))
          && Number.isInteger(Number(displayDate?.day))
          ? `${Number(displayDate.year)}-${String(Number(displayDate.month)).padStart(2, "0")}-${String(Number(displayDate.day)).padStart(2, "0")}`
          : null;
        const intervalDateIso = typeof day?.interval?.startTime === "string"
          ? day.interval.startTime.slice(0, 10)
          : null;
        const date = isIsoDate(displayDateIso) ? displayDateIso : intervalDateIso;
        const hiRaw = Number(day?.maxTemperature?.degrees);
        const loRaw = Number(day?.minTemperature?.degrees);
        if (!isIsoDate(date) || !Number.isFinite(hiRaw) || !Number.isFinite(loRaw)) return [];
        const daytimeRain = Number(day?.daytimeForecast?.precipitation?.probability?.percent);
        const nighttimeRain = Number(day?.nighttimeForecast?.precipitation?.probability?.percent);
        const availableRain = [daytimeRain, nighttimeRain].filter(Number.isFinite);
        const rain = availableRain.length
          ? Math.max(0, Math.min(100, Math.round(Math.max(...availableRain))))
          : 0;
        return [{
          date,
          hi: Math.round(hiRaw),
      ...
      if (!forecast.length) {
        logger.error("Google Weather returned no usable forecast days");
        return { status: "unavailable", reason: "invalid_or_empty_forecast", forecast: [], checkedAt: now().toISOString() };
      }
      return { status: "success", forecast, checkedAt: now().toISOString() };
    } catch (error) {
  ...
  async function fetchBlogContent(topic) {
    try {
      ...
      const text = html
        ...
        .replace(/\s+/g, " ")
        .trim()
        // The fireworks countdown schedule cards occur beyond the intro copy.
        .substring(0, topic === "fireworks" ? 9000 : 3500);
  ...
  async function fetchBeachDeals({ arrival = null, departure = null, month = null, unit = null, limit = 3 } = {}) {
    try {
      const response = await fetchWithTimeout(fetchImpl, "https://deals.destincondogetaways.com/beach-deals", {
        headers: { "User-Agent": "DestinyBlue/3.0 beach-deals-reader" },
      }, 8000);
      if (!response.ok) return { status: "unavailable", deals: [], reason: `http_${response.status}` };
      const html = await response.text();
      const match = html.match(/<script[^>]+id=["']__NEXT_DATA__["'][^>]*>([\s\S]*?)<\/script>/i);
      if (!match) return { status: "unavailable", deals: [], reason: "next_data_missing" };
      const payload = JSON.parse(match[1]);
      const rawDeals = Array.isArray(payload?.props?.pageProps?.deals) ? payload.props.pageProps.deals : [];
      const active = rawDeals.filter(deal => !deal?.purchased
        && ["707", "1006"].includes(String(deal?.unit))
        && /^\d{4}-\d{2}-\d{2}$/.test(String(deal?.arrival))
        && /^\d{4}-\d{2}-\d{2}$/.test(String(deal?.departure))
        && Number.isFinite(Number(deal?.dropPct))
        && Number(deal.dropPct) >= 3
        && Number(deal.dropPct) <= 60
        && Number(deal?.fromPrice) > Number(deal?.toPrice)
        && Number(deal?.toPrice) > 0)
        .filter(deal => !unit || String(deal.unit) === String(unit))
        .filter(deal => !month || String(deal.arrival).startsWith(`${month}-`))
        .map(deal => ({
          unit: String(deal.unit),
          arrival: String(deal.arrival),
          departure: String(deal.departure),
          nights: Number(deal.nights),
          dropPct: Number(deal.dropPct),
          fromPrice: Number(deal.fromPrice),
          toPrice: Number(deal.toPrice),
          totalSavings: Number(deal.totalSavings),
        }));
      const exact = arrival && departure
        ? active.filter(deal => deal.arrival === arrival && deal.departure === departure)
        : [];
      let selected = exact;
      let matchType = exact.length ? "exact" : arrival && departure ? "nearby" : month ? "month" : "featured";
      if (!selected.length && arrival && departure) {
        const targetArrival = Date.parse(`${arrival}T12:00:00Z`);
        const targetDeparture = Date.parse(`${departure}T12:00:00Z`);
        selected = [...active].sort((left, right) => {
          const leftDistance = Math.abs(Date.parse(`${left.arrival}T12:00:00Z`) - targetArrival)
            + Math.abs(Date.parse(`${left.departure}T12:00:00Z`) - targetDeparture);
          const rightDistance = Math.abs(Date.parse(`${right.arrival}T12:00:00Z`) - targetArrival)
            + Math.abs(Date.parse(`${right.departure}T12:00:00Z`) - targetDeparture);
          return leftDistance - rightDistance || right.dropPct - left.dropPct;
        });
      }
      if (!arrival || !departure) selected = active;
      const safeLimit = Math.max(1, Math.min(5, Number(limit) || 3));
      return {
        status: "success",
        matchType,
        deals: selected.slice(0, safeLimit),
        checkedAt: now().toISOString(),
        source: "https://deals.destincondogetaways.com/beach-deals",
      };
    } catch (error) {
      return { status: "unavailable", deals: [], reason: error.message };
    }
  }
  ...
  async function logToSheets(sessionId, guestMessage, destinyReply, datesAsked, availabilityStatus, alertSummary = "", evaluation = null) {
    try {
      const sheetId = env.GOOGLE_SHEET_ID;
      ...
      const row = [
        timestamp, sessionId || "", guestMessage, destinyReply, datesAsked || "", availabilityStatus || "", alertSummary, "",
        evaluation?.overall ?? "", evaluation?.accuracy ?? "", evaluation?.completeness ?? "", evaluation?.humanTone ?? "",
        evaluation?.toolLinkCompliance ?? "", evaluation?.safety ?? "", evaluation?.priority ?? "",
        evaluation?.failureReason ?? "", evaluation?.improvement ?? "",
      ];
  ...
  async function logVoiceEvent(event) {
    try {
      const sheetId = env.GOOGLE_SHEET_ID;
      if (!sheetId) return { ok: false, reason: "missing_configuration" };
      const accessToken = await getSheetsToken();
      ...
      if (!eventId || !sessionId) return { ok: false, reason: "missing_event_identity" };

      // Stable event IDs make browser retries and Realtime reconnects idempotent.
      // Read only the metadata column before appending; never depend on process-local
      // memory because Vercel may execute retries on different instances.
      const existingRes = await fetchWithTimeout(fetchImpl, `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Sheet1!G:G`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      }, 8000);
      if (!existingRes.ok) return { ok: false, reason: "dedupe_read_failed", status: existingRes.status };
      const existingRows = (await existingRes.json()).values || [];
      const duplicate = existingRows.some(row => {
        try { return JSON.parse(row?.[0] || "{}").voiceEventId === eventId; }
        catch (_) { return false; }
      });
      if (duplicate) return { ok: true, duplicate: true };

      const role = event.role === "user" || event.role === "assistant" ? event.role : "system";
      ...
      const metadata = JSON.stringify({
        ...
        clientTimestamp: String(event.clientTimestamp || ""),
        ...
        callEpoch: Number.isInteger(event.callEpoch) ? event.callEpoch : null,
        transportId: String(event.transportId || ""),
        responseId: String(event.responseId || ""),
        requestToken: String(event.requestToken || ""),
        leaseKind: String(event.leaseKind || ""),
        coordinatorState: String(event.coordinatorState || ""),
        playbackState: String(event.playbackState || ""),
        generationState: String(event.generationState || ""),
        candidateId: String(event.candidateId || ""),
        taskId: String(event.taskId || ""),
        waveId: String(event.waveId || ""),
        reason: String(event.reason || ""),
        queueDepth: Number.isInteger(event.queueDepth) ? event.queueDepth : null,
        voiceModel: String(event.voiceModel || ""),
        clearInitiator: String(event.clearInitiator || ""),
        traceId: String(event.traceId || ""),
        subrequestId: String(event.subrequestId || ""),
        requestedRoute: String(event.requestedRoute || ""),
        executedRoute: String(event.executedRoute || ""),
        domainStatus: String(event.domainStatus || ""),
        knowledgeRevision: String(event.knowledgeRevision || ""),
        knowledgeSource: String(event.knowledgeSource || ""),
        cacheState: String(event.cacheState || ""),
        fallbackReason: String(event.fallbackReason || ""),
        httpStatus: Number.isInteger(event.httpStatus) ? event.httpStatus : null,
        nestedLatencyMs: Number.isFinite(event.nestedLatencyMs) ? event.nestedLatencyMs : null,
        nestedToolNames: Array.isArray(event.nestedToolNames) ? event.nestedToolNames.map(String) : [],
        resolvedIds: Array.isArray(event.resolvedIds) ? event.resolvedIds.map(String) : [],
        unresolvedIds: Array.isArray(event.unresolvedIds) ? event.unresolvedIds.map(String) : [],
        systemText: role === "system" ? text : "",
      });
      const row = [
        timestamp,
        sessionId,
        ...
        role === "assistant" ? text : "",
        "",
        `VOICE_${String(event.eventType || "UNKNOWN").toUpperCase()}`,
        metadata,
        "",
      ];
      const appendRes = await fetchWithTimeout(fetchImpl, `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Sheet1!A1:append?valueInputOption=USER_ENTERED`, {
        method: "POST",
        headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
        body: JSON.stringify({ values: [row] }),
      }, 8000);
      return { ok: appendRes.ok, status: appendRes.status, duplicate: false };
    } catch (error) {
      logger.error("Voice event logging error:", error.message);
      return { ok: false, reason: error.message };
    }
  }

  async function logVoiceEvents(events) {
    try {
      const sheetId = env.GOOGLE_SHEET_ID;
      if (!sheetId) return { ok: false, reason: "missing_configuration" };
      const accessToken = await getSheetsToken();
      if (!accessToken) return { ok: false, reason: "auth_failed" };
      const validEvents = (Array.isArray(events) ? events : []).filter(event => String(event?.eventId || "").trim() && String(event?.sessionId || "").trim());
      if (!validEvents.length) return { ok: false, reason: "missing_event_identity" };
      const existingRes = await fetchWithTimeout(fetchImpl, `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Sheet1!G:G`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      }, 8000);
      if (!existingRes.ok) return { ok: false, reason: "dedupe_read_failed", status: existingRes.status };
      const existingIds = new Set(((await existingRes.json()).values || []).map(row => {
        try { return JSON.parse(row?.[0] || "{}").voiceEventId; } catch (_) { return ""; }
      }).filter(Boolean));
      const batchIds = new Set();
      const freshEvents = validEvents.filter(event => {
        const eventId = String(event.eventId).trim();
        if (existingIds.has(eventId) || batchIds.has(eventId)) return false;
        batchIds.add(eventId);
        return true;
      });
      if (!freshEvents.length) return { ok: true, duplicate: true, stored: 0 };
      const timestamp = now().toLocaleString("en-US", { timeZone: "America/Chicago" });
      const rows = freshEvents.map(event => {
        const role = event.role === "user" || event.role === "assistant" ? event.role : "system";
        const text = String(event.text || "");
        const metadata = JSON.stringify({
          schemaVersion: Number.isInteger(event.schemaVersion) ? event.schemaVersion : 1,
          voiceEventId: String(event.eventId || ""), callId: String(event.callId || ""), turnId: String(event.turnId || ""),
          eventType: String(event.eventType || "unknown"), role, source: "voice-lab", interrupted: event.interrupted === true,
          toolName: String(event.toolName || ""), toolStatus: String(event.toolStatus || ""), providerEventId: String(event.providerEventId || ""),
          clientTimestamp: String(event.clientTimestamp || ""), latencyMs: Number.isFinite(event.latencyMs) ? event.latencyMs : null,
          sequence: Number.isInteger(event.sequence) ? event.sequence : null, monotonicMs: Number.isFinite(event.monotonicMs) ? event.monotonicMs : null,
          callEpoch: Number.isInteger(event.callEpoch) ? event.callEpoch : null, transportId: String(event.transportId || ""),
          responseId: String(event.responseId || ""), requestToken: String(event.requestToken || ""), leaseKind: String(event.leaseKind || ""),
          coordinatorState: String(event.coordinatorState || ""), playbackState: String(event.playbackState || ""), generationState: String(event.generationState || ""),
          candidateId: String(event.candidateId || ""), taskId: String(event.taskId || ""), waveId: String(event.waveId || ""),
          reason: String(event.reason || ""), queueDepth: Number.isInteger(event.queueDepth) ? event.queueDepth : null,
          voiceModel: String(event.voiceModel || ""), clearInitiator: String(event.clearInitiator || ""),
          traceId: String(event.traceId || ""), subrequestId: String(event.subrequestId || ""),
          requestedRoute: String(event.requestedRoute || ""), executedRoute: String(event.executedRoute || ""),
          domainStatus: String(event.domainStatus || ""), knowledgeRevision: String(event.knowledgeRevision || ""),
          knowledgeSource: String(event.knowledgeSource || ""), cacheState: String(event.cacheState || ""),
          fallbackReason: String(event.fallbackReason || ""), httpStatus: Number.isInteger(event.httpStatus) ? event.httpStatus : null,
          nestedLatencyMs: Number.isFinite(event.nestedLatencyMs) ? event.nestedLatencyMs : null,
          nestedToolNames: Array.isArray(event.nestedToolNames) ? event.nestedToolNames.map(String) : [],
          resolvedIds: Array.isArray(event.resolvedIds) ? event.resolvedIds.map(String) : [],
          unresolvedIds: Array.isArray(event.unresolvedIds) ? event.unresolvedIds.map(String) : [],
        ...
        return [timestamp, String(event.sessionId || ""), role === "user" ? text : "", role === "assistant" ? text : "", "", `VOICE_${String(event.eventType || "UNKNOWN").toUpperCase()}`, metadata, ""];
      });
      const appendRes = await fetchWithTimeout(fetchImpl, `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Sheet1!A1:append?valueInputOption=USER_ENTERED`, {
        method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ values: rows }),
      }, 8000);
      return { ok: appendRes.ok, status: appendRes.status, duplicate: false, stored: appendRes.ok ? rows.length : 0 };
    } catch (error) {
      logger.error("Voice event batch logging error:", error.message);
      return { ok: false, reason: error.message };
    }
  }
