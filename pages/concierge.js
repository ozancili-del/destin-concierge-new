  ...
  async function send(e) {
    ...
    try {
      const r = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ messages: [...log, userMsg], sessionId: sessionIdRef.current, alertSent, pendingRelay, ozanAcked, ozanAckType, pageSource: typeof window !== "undefined" && window.location.pathname.includes("ai-concierge") ? "ai-concierge" : (typeof sessionStorage !== "undefined" && sessionStorage.getItem('db_source')) || null, sawBanner: typeof localStorage !== "undefined" ? (localStorage.getItem('db_saw_banner') || sessionStorage.getItem('db_saw_banner')) : null, guestBooking: guestBookingRef.current || null })
      });
      const data = await r.json();
