...
export function evidenceBasedRecoveryFallback({ state, latestUser = "", toolResults = [], violations = [] }) {
  ...
  if (bedroomMismatch && Number(state?.booking?.bedroomsRequested || 0) >= 2) {
    ...
    if (availability) {
      ...
      const missing = new Set(state?.awaiting || []);
      const needsDates = !state?.booking?.arrival || !state?.booking?.departure
        || missing.has("arrival") || missing.has("departure");
      const needsAdults = !Number.isInteger(state?.booking?.adults) || missing.has("adults");
      const needsChildren = !Number.isInteger(state?.booking?.children) || missing.has("children");
      const needed = [];
      if (needsDates) needed.push("check-in and check-out dates");
      if (needsAdults) needed.push("number of adults");
      if (needsChildren) needed.push("number of children");
      if (needed.length) lines.push(`Send the ${needed.join(" and ")}, and I’ll check the options that genuinely fit.`);
    }
    return lines.join("\n\n");
  }
