  ...
  try {
    ...
    if (name === "check_availability") {
      ...
      if (party.needsTwoUnits) {
        ...
        return toolResult({ name, kind: "booking", status: urls.length === 2 ? "success" : "unavailable", data: { query: { ...input.dates, adults: party.adults, children: party.children }, holidayStay: input.holidayStay || null, assumedChildrenZero: input.assumedChildrenZero === true, needsTwoUnits: true, split, units, checkedAt: new Date(now).toISOString() }, urls, facts: patch.verified.facts, statePatch: patch });
      }
      ...
      return toolResult({
        ...
        data: { query: { ...input.dates, adults: party.adults, children: party.children }, holidayStay: input.holidayStay || null, assumedChildrenZero: input.assumedChildrenZero === true, units, alternatives, priceDrops: priceDrops.drops, checkedAt: new Date(now).toISOString() },
        urls,
        facts,
        statePatch: patch,
      });
    ...
    if (name === "get_local_guide") {
      ...
      if (["restaurants", "restaurants2"].includes(args.topic)) {
        const published = typeof services.fetchPublishedKnowledge === "function"
          ? await services.fetchPublishedKnowledge(String(latestUser || "restaurants near Pelican Beach Resort").slice(0, 300), [args.topic], 6)
          : { source: "legacy", snippets: [], urls: [] };
        const guide = await services.fetchBlogContent(args.topic);
        const urls = guide.url ? [guide.url] : [];
        const publishedFacts = published.source === "published" ? published.snippets.map(item => item.text) : [];
        const facts = publishedFacts.length ? publishedFacts : (guide.content ? [guide.content] : []);
        return toolResult({
          name,
          kind: "guide",
          ok: facts.length > 0,
          status: facts.length > 0 ? "success" : guide.status,
          data: { ...guide, knowledgeSource: publishedFacts.length ? "published" : "website", knowledgeRevision: published.revision || null },
          urls: [...new Set([...urls, ...(published.urls || [])])],
          facts,
          statePatch: { verified: { blogUrls: urls, facts } },
        });
      }
    ...
    if (name === "get_business_knowledge") {
      const published = typeof services.fetchPublishedKnowledge === "function"
        ? await services.fetchPublishedKnowledge(
          String(args.query || latestUser || "").slice(0, 300),
          Array.isArray(args.topics) ? args.topics : [],
          Number(args.limit) || 8,
        )
        : { source: "legacy", snippets: [], urls: [] };
      const result = published.source === "published" && published.snippets.length ? published : searchBusinessKnowledge({
      ...
      return toolResult({
        ...
        data: { query: result.query, topics: result.topics, snippets: result.snippets, knowledgeSource: result.source || "legacy", knowledgeRevision: result.revision || null },
        urls: result.urls,
...
export async function runAgentTurn({
  ...
  // Private shared-runtime injection. The public call path keeps its existing defaults.
  const runTool = runtime?.executeTool || executeTool;
  let workingState = normalizeState(state || createDefaultState());
  const safety = runtime?.safetyBackstops === false ? {state:workingState,toolResults:[]} : await applySafetyBackstops({ state: workingState, latestUser, services, sessionId, now });
  ...
  for (let round = 0; round < maxToolRounds; round += 1) {
    ...
    try {
      response = await callResponses(openai, {
        model,
        input,
        tools: runtime?.tools || RESPONSE_TOOL_DEFINITIONS,
        tool_choice: "auto",
    ...
    const roundResults = await Promise.all(calls.map(async (call) => {
      ...
      return withTimeout(runTool(call.name, args, {
        openai,
        model,
        services,
        state: workingState,
  ...
  for (const category of recommendedCoveredCategories) {
    ...
    const result = await withTimeout(runTool("get_activity_options", {
      category,
      date_text: null,
