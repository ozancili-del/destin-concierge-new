...
function hostedPreview(req,env){
  const host=String(req.headers.host||'');
  return env.VERCEL_ENV==='preview'&&req.headers.origin===`https://${host}`&&/\.vercel\.app$/i.test(host)&&!!req.headers['x-vercel-id'];
}
...
export function createPrivateRealtimeHandler({env=process.env,fetchImpl=fetch}={}){
  return async(req,res)=>{
    ...
    const isHostedPreview=hostedPreview(req,env);
    if(!isHostedPreview&&!isPrivatePeer(req,env))return res.status(404).json({error:'not_found'});
    if(req.method!=='POST')return res.status(405).json({error:'post_required'});
    if(!isHostedPreview&&req.headers.origin!==`http://${req.headers.host}`)return res.status(403).json({error:'origin_denied'});
    if(env.DESTINY_PRIVATE_BROKER_URL)return res.status(409).json({error:'bounded_voice_required'});
    if(env.DESTINY_ORDERED_MEMORY==='1'&&env.DESTINY_PRIVATE_MODEL_CALLS!=='1')return res.status(503).json({error:'private_model_disabled'});
    if((!isHostedPreview&&env.DESTINY_PRIVATE_MODEL_CALLS!=='1')||!env.OPENAI_API_KEY)return res.status(503).json({error:'private_model_disabled'});
    ...
    try{
      ...
      const session={type:'realtime',model:VOICE_MODEL,output_modalities:['audio'],max_output_tokens:1600,
        ...
        audio:{input:{transcription:{model:'gpt-4o-mini-transcribe',...(env.DESTINY_ORDERED_MEMORY==='1'?{}:{language:'en'})},noise_reduction:{type:'near_field'},turn_detection:null},output:VOICE_OUTPUT},tools:[],tool_choice:'none'};
