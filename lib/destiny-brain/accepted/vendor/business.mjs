...
export const STATIC_URLS = Object.freeze({
  ...
  reviews: "https://www.destincondogetaways.com/destin-condo-rental-reviews",
  liveBeachCam: "https://www.destincondogetaways.com/beach-cam",
  weatherLive: "https://www.destincondogetaways.com/blog/destinweather",
  tripPlanner: "https://www.destincondogetaways.com/destin-vacation-itinerary-planner",
  beachDeals: "https://www.destincondogetaways.com/destin-condo-deals",
  sunbird: "https://www.destincondogetaways.com/destin-snowbird-rentals",
});

export const OFFER_INQUIRY_URL = "https://www.destincondogetaways.com/destin-condo-special-offers";
...
export const BLOG_URLS = Object.freeze({
  ...
  events:       "https://www.destincondogetaways.com/blog/destin-events",
...
export const TRIPSHOCK_CATEGORIES = Object.freeze({
  ...
  gulfarium:    "@product:gulfarium-marine-adventure-park-admission-tickets/details/1453",
  sciencecenter:"@product:emerald-coast-science-center-museum-admission-tickets/details/2367",
  boattour:     "boat-tours",
});

// Canonical recommendation vocabulary used only as an affiliate-compliance guardrail.
// The model still chooses and explains activities; this detector ensures that when its
// final prose names a supported TripShock activity, the corresponding link-builder runs.
const TRIPSHOCK_RECOMMENDATION_PATTERNS = Object.freeze({
  dolphin: /\bdolphin (?:cruise|tour|experience|excursion)s?\b/i,
  fishing: /\bfishing (?:charter|trip|excursion)s?\b/i,
  jetski: /\b(?:jet ski|jet-ski|waverunner)(?: rental| tour| ride)?s?\b/i,
  pontoon: /\bpontoon(?: boat)?(?: rental| tour)?s?\b/i,
  boat: /\bboat rentals?\b/i,
  parasail: /\bparasail(?:ing)?\b/i,
  crabisland: /\bcrab island\b/i,
  snorkel: /\bsnorkel(?:ing)?(?: tour| trip)?s?\b/i,
  sunset: /\bsunset cruise?s?\b/i,
  pirate: /\bpirate cruise?s?\b/i,
  kayak: /\bkayak(?:ing| rental)?s?\b/i,
  paddleboard: /\b(?:paddleboard|paddle board|stand-up paddleboard)(?:ing| rental)?s?\b/i,
  fireworks: /\bfireworks? cruise?s?\b/i,
  tiki: /\btiki boat(?: cruise| tour)?s?\b/i,
  banana: /\bbanana boat(?: ride)?s?\b/i,
  photographer: /\b(?:beach photographer|beach photo(?:graphy| session)|family beach photos?)\b/i,
  gulfarium: /\bGulfarium(?: Marine Adventure Park)?\b/i,
  sciencecenter: /\bEmerald Coast Science Center\b/i,
  boattour: /\bboat tours?\b/i,
});

export function detectTripShockRecommendations(text) {
  const value = String(text || "");
  return Object.entries(TRIPSHOCK_RECOMMENDATION_PATTERNS)
...
export function resolveHolidayStay(holiday, now = new Date(), requestedYear = null) {
  ...
  const explicitYear = Number(requestedYear);
  let year = Number.isInteger(explicitYear) && explicitYear >= 2000 && explicitYear <= 2100
    ? explicitYear
    : Number(today.slice(0, 4));
  let holidayDate = occurrence(year);
  if (!Number.isInteger(explicitYear) && holidayDate < today) holidayDate = occurrence(++year);
  return {
    holiday,
    holidayDate,
...
export function detectVagueWeek(text) {
  return /\b(first|second|third|fourth|last|middle|mid|early|late)\s+(week|part)\s+of\s+(january|february|march|april|may|june|july|august|september|october|november|december)\b/i.test(String(text || ""));
}

export function parseDateAdjustment(text, currentDates) {
  if (!currentDates?.arrival || !currentDates?.departure) return null;
  const t = String(text || "").toLowerCase();
  // A bare "make it N days later/earlier" does not identify whether "it"
  // means check-in, check-out, or the whole stay. Fail closed so the agent asks
  // the owner-approved clarification instead of silently changing both dates.
  if (/^\s*(?:please\s+)?make\s+it\s+(?:one|two|three|four|five|six|seven|eight|nine|ten|\d+)\s+days?\s+(?:later|earlier|sooner|before|after)\s*[.!?]*\s*$/i.test(t)) return null;
...
export function buildTripShockLink(category, dates) {
  ...
  if (slug.startsWith("@product:")) return `${TRIPSHOCK_BASE}/${slug.slice("@product:".length)}/?${TRIPSHOCK_AFF}`;
...
export function detectOwnerChatRequest(text) {
  return /@ozan|talk (?:to|with) ozan|speak (?:to|with) ozan|chat with ozan|connect me.*ozan|(?:invite|add|bring|get|have|let)\s+(?:ozan|the owner|owner|a human|a real person)\s+(?:into|in|join|to)\b|(?:can|could|will|would)\s+(?:ozan|the owner|owner|a human|a real person)\s+join\b|owner.*chat|human.*chat|real person|talk (?:to|with) the owner|speak (?:to|with) the owner/i.test(String(text || ""));
}
...
export function validateReply({ reply, allowedUrls, toolResults = [], state, latestUser = "", requireCurrentTurnUrls = true }) {
  ...
  const bookingToolUsed = toolResults.some(result => result?.kind === "booking");
  const claimsBookingAction = /\b(?:i|we)(?:['’]ll|\s+can|\s+could|\s+will)?(?:\s*[:–-]\s*|\s+)(?:hold|reserve|book|complete|finalize)\b[^.!?\n]{0,90}\b(?:unit|condo|reservation|booking|one of these|it|this)\b/i.test(text)
    || /\b(?:want|like|need)\s+me\s+to\s+(?:hold|reserve|book|complete|finalize)\b[^.!?\n]{0,90}\b(?:unit|condo|reservation|booking|one of these|it|this)\b/i.test(text)
    || /\b(?:i|we)(?:['’]ll|\s+can|\s+could|\s+will)?\s+help\s+(?:you\s+)?(?:reserve|book|complete|finalize|reserving|booking)\b[^.!?\n]{0,90}\b(?:unit|condo|reservation|booking|one|it|this)\b/i.test(text)
    || /\b(?:would\s+you\s+like|do\s+you\s+want|want)\s+(?:me\s+to\s+)?(?:help\s+(?:you\s+)?)?(?:hold|reserve|book|complete|finalize|reserving|booking)\b[^.!?\n]{0,90}\b(?:unit|condo|reservation|booking|one|it|this|(?:that\s+)?link)\b/i.test(text);
  if (bookingToolUsed && claimsBookingAction) {
    violations.push({ code: "unsupported_booking_action", detail: "The concierge can check availability and provide secure links, but cannot hold, reserve, or complete a booking for the guest." });
  }

  const beachConditions = [...toolResults].reverse().find(result => result?.name === "get_beach_conditions");
  if (beachConditions) {
    if (/\b(?:the\s+)?(?:water|gulf|beach|conditions?)\s+(?:is|are|looks?|should be)\s+(?:completely\s+|generally\s+)?safe\b|\bsafe\s+to\s+(?:swim|enter|go in)\b/i.test(text)) {
      violations.push({ code: "unsafe_water_assurance", detail: "Official conditions may be reported, but the concierge must never declare Gulf water safe." });
    }
    if (beachConditions.data?.flag?.value === "Water Closed to Public" && !/\b(?:water|gulf|beach)\b[^.!?\n]{0,80}\bclosed\b|\bdo not enter\b|\bstay out of (?:the )?water\b/i.test(text)) {
      violations.push({ code: "water_closure_omitted", detail: "The official Destin Fire water closure must be stated prominently." });
    }
    if (/^high\b/i.test(String(beachConditions.data?.surf?.ripCurrentRisk || "")) && !/\bhigh\b[^.!?\n]{0,40}\brip[- ]current risk\b|\brip[- ]current risk\b[^.!?\n]{0,40}\bhigh\b/i.test(text)) {
      violations.push({ code: "high_rip_risk_omitted", detail: "The current high NWS rip-current risk must be stated prominently." });
    }
  }
  ...
  const activityLinkBuilt = toolResults.some(result => result?.name === "get_activity_options" && result?.ok !== false);
  const claimsLiveActivityAvailability = /\b(?:dolphin|sunset|pirate|pontoon|parasail|jet[- ]?ski|photograph|snorkel|fishing|activities?|tours?|cruises?|rentals?)[^.!?\n]{0,80}\b(?:is|are|looks?|remain)\s+(?:currently\s+)?available\b/i.test(text);
  if (activityLinkBuilt && claimsLiveActivityAvailability) {
    violations.push({ code: "unverified_activity_availability", detail: "A TripShock link does not prove activity inventory. Direct the guest to open it to check current prices, times, and availability." });
  }
  ...
  const availablePriceDrops = (availability?.data?.priceDrops || []).filter(drop =>
    availability?.data?.units?.some(unit => unit.unit === String(drop.unit) && unit.available === true)
  );
  for (const drop of availablePriceDrops) {
    const unitMentioned = new RegExp(String.raw`(?:unit\\s*)?${drop.unit}\\b`, "i").test(text);
    const percentMentioned = new RegExp(String.raw`\\b${drop.dropPct}\\s*%`).test(text);
    const reductionMentioned = /\\b(?:price|rate)\\b[^.!?\\n]{0,60}\\b(?:drop|dropped|reduced|reduction|lower|down)\\b|\\b(?:drop|dropped|reduced|reduction|lower|down)\\b[^.!?\\n]{0,60}\\b(?:price|rate)\\b/i.test(text);
    if (!(unitMentioned && percentMentioned && reductionMentioned)) {
      violations.push({ code: "verified_price_drop_omitted", detail: `Mention Unit ${drop.unit}'s verified ${drop.dropPct}% price reduction naturally, without artificial scarcity.` });
    }
  }
