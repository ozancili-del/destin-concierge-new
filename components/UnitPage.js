...
export default function UnitPage({ unit }) {
  ...
  const canonical = `${liveSite}/pelican-beach-resort-unit-${unit.number}`;
  const photos = unit.schema.image;
  const viewPhotoIndex = photos.length > 5 ? 5 : 1;
  ...
  useEffect(() => {
    ...
    const closeButton = dialog?.querySelector('[aria-label="Close photo gallery"]');
    closeButton?.focus();
    const onKey = (event) => {
  ...
  return <div className={styles.page}>
    ...
    <main>
      ...
      <nav className={styles.journey} aria-label="Continue planning this Destin stay"><div><p className={styles.kicker}>Continue with useful details</p><h2>Place this exact condo in the rest of your trip.</h2><p>Review the building and location, compare the other available home, then use the local guides only for the parts of the vacation you still need to plan.</p></div><div className={styles.journeyGrid}><a href="/destin-vacation-rentals-by-owner"><span>Vacation rentals</span><strong>Compare the available beachfront homes</strong></a><a href="https://www.mypelicanbeach.com/unit-707-vs-unit-1006"><span>Independent condo guide</span><strong>Compare Units 707 and 1006 on MyPelicanBeach</strong></a><a href="/pelican-beach-resort-destin"><span>Resort &amp; location</span><strong>Beach access, pools and central Destin</strong></a><a href="/destin-vacation-itinerary-planner"><span>Trip planner</span><strong>Build a day-by-day itinerary</strong></a><a href="/blog/destinweather"><span>Weather &amp; Gulf</span><strong>Plan around current conditions</strong></a></div></nav>
    ...
    <SiteFooter />
    {lightbox !== null && <div ref={lightboxRef} className={styles.lightbox} role="dialog" aria-modal="true" aria-label={`Unit ${unit.number} photo gallery`}><button className={styles.close} onClick={() => setLightbox(null)} aria-label="Close photo gallery">×</button><button className={styles.previous} onClick={() => setLightbox((lightbox - 1 + photos.length) % photos.length)} aria-label="Previous photo">‹</button><figure><img src={photos[lightbox]} alt={photoDescription(unit, lightbox)} /><figcaption>{photoDescription(unit, lightbox)} <span>{lightbox + 1} / {photos.length}</span></figcaption></figure><button className={styles.next} onClick={() => setLightbox((lightbox + 1) % photos.length)} aria-label="Next photo">›</button></div>}
