...
function buildSubrequests(input, interpretation) {
  ...
  if (restaurantCategory || flags.shopping) {
    ...
    if (flags.liveVenueStatus) {
      add({ intent: "venue_status", route: "live", capability: "venue_status.read", fields: ["opening_status"], temporalKind: "current", dependsOn: interpretation.entities.subjectIds.length ? [] : ["s1"] });
    }
    return result;
  }
  ...
  add({ intent: "unknown", route: "clarify", fields: ["intent"] });
  return result;
}
