...
- "3 more friends joining us" = 3`
    ...
    // ── DETERMINISTIC ORIGIN RESOLUTION (pre-prompt) ───────────────────────
    // Resolve the guest's origin airport in CODE before GPT writes anything.
    // If we can resolve it, we build the link ourselves and hard-instruct GPT
    // not to ask which airport — removing the decision from GPT entirely.
    const vagueAirportReplyPre = /^\s*(any|anyone|any one|any of them|whichever|whatever|either|doesn'?t matter|dont matter|no preference|you (pick|choose|decide)|up to you|surprise me|closest|cheapest|the (main|major|biggest) one)\s*[.!]*\s*$/i.test(String(lastUser || "").trim());
    const allUserTextForOrigin = [...messages].filter(m => m.role === "user").map(m => m.content).join(" ");
    const preResolvedOrigin = extractOrigin(String(lastUser || "")) || (vagueAirportReplyPre ? extractOrigin(allUserTextForOrigin) : null);
    // Cities where guests may expect a choice — we pick the main one and say so.
    const MULTI_AIRPORT_MAIN = { ORD: "Chicago O'Hare", JFK: "New York JFK", IAD: "Washington Dulles", IAH: "Houston Bush", DFW: "Dallas/Fort Worth", SFO: "San Francisco" };
    const preOriginIsMultiCity = preResolvedOrigin ? Object.prototype.hasOwnProperty.call(MULTI_AIRPORT_MAIN, preResolvedOrigin) : false;
...
${flightLink
  ? (preResolvedOrigin
      ? `✅ ORIGIN ALREADY RESOLVED BY THE SYSTEM: ${preResolvedOrigin}${preOriginIsMultiCity ? ` (${MULTI_AIRPORT_MAIN[preResolvedOrigin]})` : ""}.
The flight link is ALREADY BUILT and will be appended below your message automatically.
...
Tell guest warmly that neither unit is free for the full stay, but offer these shorter alternatives. Your 10% direct booking discount is already applied! 🎉`;
              ...
              const earlier707 = (u707.earlierArrival && u707.earlierArrival >= dates.arrival) ? (() => {
                ...
                const arrPhrase = u707.earlierArrival === nowLocal ? "TODAY (use the word TODAY in bold, do NOT write the date)" : daysEarlier === 1 ? "TOMORROW (use the word TOMORROW in bold, do NOT write the date)" : fmt(u707.earlierArrival);
              ...
              const earlier1006 = (u1006.earlierArrival && u1006.earlierArrival >= dates.arrival) ? (() => {
                ...
                const fmt = iso => new Date(iso+"T12:00:00Z").toLocaleDateString("en-US",{month:"long",day:"numeric"});
                const arrPhrase = u1006.earlierArrival === nowLocal ? "TODAY (use the word TODAY in bold, do NOT write the date)" : daysEarlier === 1 ? "TOMORROW (use the word TOMORROW in bold, do NOT write the date)" : fmt(u1006.earlierArrival);
...
WEATHER DATA UNAVAILABLE: Real-time weather could not be fetched. Do NOT guess or invent temperatures. Tell the guest honestly: "I don't have live weather data at the moment — for the most accurate Destin forecast I'd recommend checking weather.com. What I can say is that February in Destin typically sees highs in the mid-50s to low 60s°F, and the Gulf is quite chilly — our indoor heated pool is perfect this time of year!" Do NOT confidently state specific temperatures you are not sure about.`;
      ...
      if (blogResult) {
        const interactiveHints = {
          besttime:    `\n\nINTERACTIVE TOOL: This blog page also has a "Find Your Perfect Month" quiz — 4 quick questions that give a personalized Destin timing recommendation. At the end of your answer, mention it casually: something like "...and there's actually a quick quiz on that page that picks the perfect month for YOUR trip specifically — worth a try!" Plain text link only: ${blogResult.url}`,
          fireworks:   `\n\nINTERACTIVE TOOL: This blog page has a live fireworks countdown for every show plus an interactive map showing exact locations and best viewing spots. At the end of your answer mention it naturally: something like "...the page also has a live countdown and interactive map so you can see exactly where each show is and plan your spot." Plain text link only: ${blogResult.url}`,
...
` : "";
    ...
    const conciergeEmailContext = pageSource === "ai-concierge" ? `
🎯 CONCIERGE PAGE GUEST — 5% EMAIL UNLOCK:
After sending booking links, you may offer an extra 5% discount in exchange for their email.
If the guest provides an email address at any point:
- Validate it (must have @ and real domain)
- If valid: reveal code BLUE — "You're all set! 🎉 Use code BLUE at checkout for your extra 5% on top of your automatic 10%."
- If fake/invalid: say warmly "Hmm that doesn't look quite right — want to try again? 😊"
- NEVER reveal BLUE before a valid email is given.
` : "";
...
PAYMENTS: A non-refundable 20% deposit is collected at booking. The remaining balance is due 30 days before arrival.
...
CANCELLATION: Cancelling more than 30 days before arrival forfeits the non-refundable 20% deposit. No refund is provided within 30 days of arrival. The secure checkout terms control the reservation.
...
LIVE BEACH & WATER CONDITIONS RULE:
There are two types of weather questions — handle them differently:
1. FORECAST questions ("will it rain today?", "how hot this week?", "is it cloudy?", "it's raining — will it stop?", "should I pack a jacket?") → answer directly from the Google Weather forecast data above. This includes rain planning and indoor activity suggestions.
2. BEACH/WATER CONDITIONS questions ("what's the Gulf water like today?", "is the water warm right now?", "what are sea conditions like?", "is it good for swimming today?", "what's the wind like on the beach?", "what are the tides today?") → do NOT use seasonal averages for these. Instead say something like: "For live beach conditions right now — water temperature, wind, humidity and today's tides — check our live widget here: https://www.destincondogetaways.com/blog/destinweather or on the live cam page: https://www.destincondogetaways.com/destin-live-beach-cam-574002656 — it updates every 6 minutes via NOAA 🌊"
KEY: Rain/forecast/air temp = answer from Google API. Gulf water/sea/tides/beach conditions right now = refer to live widget pages. Never mix the two.
...
NO REPETITION RULE: Review all your previous responses in this conversation before replying. Do not recommend any venue, restaurant, activity, or attraction you have already mentioned in this conversation — even if the guest asks for "more." If your list is exhausted, say so honestly or suggest checking local event listings. Do not share the same blog link URL more than once per conversation unless the guest explicitly requests it again or raises a new subject that requires a different blog. TripShock links are exempt from this rule — each activity recommendation may include its own TripShock link even if a TripShock link was shared earlier in the conversation.`;
    ...
    const hasBdrMismatch = wantsAvailability && !guestBooking && !bookingLinksSent &&
...
Your 10% direct booking discount is already applied! 🎉 Unit 707 availability unconfirmed — contact Ozan at (972) 357-4262 if needed 😊`;
    ...
    // Fire if the guest named an airport OR the system already pre-resolved one
    // pre-prompt (prevents the prompt promising a link the code never appends).
    if ((guestMentionedAirport || preResolvedOrigin) && dates && dates.arrival && dates.departure && adults) {
      ...
      let resolvedOrigin = preResolvedOrigin || extractOrigin(lastUser);
      if (!resolvedOrigin) {
