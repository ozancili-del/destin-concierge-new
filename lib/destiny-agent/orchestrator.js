  ...
  try {
    ...
    if (name === "search_current_events") {
      ...
      const searchAttempts = await Promise.allSettled([
        callResponses(openai, makeSearchRequest("Check Club LA, The Quarry, Mattie Kelly Arts Center, and primary ticketing listings for every matching date."), 26000, "current_events_search_ticketed"),
        callResponses(openai, makeSearchRequest("Check LuLu's Destin calendar for each requested day, plus HarborWalk Village and Fort Walton Beach official venue calendars."), 26000, "current_events_search_destin_venues"),
        callResponses(openai, makeSearchRequest("Check Baytowne Wharf and Miramar Beach official or tourism calendars, then fill any remaining nearby gaps from primary sources."), 26000, "current_events_search_baytowne"),
      ]);
