...
export function prepareMigratedArticleHtml(html = "") {
  return cleanInternalLinksInHtml(html)
    // OwnerRez article exports sometimes carried document-level SEO tags in
    // the article body. The Next.js page shell owns these tags now; retaining
    // the legacy copies creates conflicting descriptions and canonicals.
    .replace(/<meta\b[^>]*\bname\s*=\s*(["'])description\1[^>]*>/gi, "")
