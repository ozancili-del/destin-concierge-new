...
const STOP = new Set(["the", "and", "for", "with", "that", "this", "from", "your", "you", "are", "our", "have", "has", "what", "when", "where", "which", "does", "can", "will", "about", "into", "only", "guest", "guests", "destin", "near", "recommend", "some", "please", "tell", "generally", "like", "need", "other", "one", "ones", "more", "spot", "spots", "option", "options"]);
...
export function rankPublishedKnowledge(bundle, { query = "", topics = [], limit = 8, requireMatch = false } = {}) {
  ...
  const snippets = candidates
    .filter((item, index) => requireMatch ? (qTerms.size ? item.matchedTerms === bestMatchCount && bestMatchCount > 0 : selectedTopics.size > 0) : item.score > 0 || index < Math.min(2, candidates.length))
    .slice(0, cap);
