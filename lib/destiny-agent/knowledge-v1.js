// lib/destiny/knowledge.js
// Static knowledge base COPIED VERBATIM from the v1 SYSTEM_PROMPT (chat.js lines
// ~3137-3590): properties, resort facilities, policies, check-in/out, local tips,
// tone rules. The INTENT-suffix mandate and dynamic context blocks are intentionally
// NOT here — v2 replaces them with tools and code-derived intent.
...
- TripShock DOES cover: dolphin cruises, fishing charters, pontoon boat rentals, jet ski/waverunner rentals, Crab Island trips, parasailing, kayak/paddleboard, snorkeling, sunset cruises, pirate cruises, banana boat rides, fireworks cruises, tiki boats, beach photographers, boat tours, Gulfarium admission tickets, Emerald Coast Science Center admission tickets
...
// ── VERIFIED COMMERCIAL CONSTANTS — code-supplied, never model-recalled ──────
// A model inventing "20% off" is a real commercial risk. These are the only
// numbers the composer may state for discounts, injected as verified data.
export const DISCOUNTS = {
  directPct: 10,          // automatic on every direct booking
  bluePct: 5,             // extra, only after email captured via capture_lead
  combinedPct: 15,        // 10 + 5 stated together
  monthlyPct: 40,         // 28+ nights, Nov 1 2026 - Feb 28 2027
  platformFeeUpToPct: 22, // what booking platforms can charge
};

export const CONTACTS = {
  ozanPhone: "(972) 357-4262",
  ozanEmail: "ozan@destincondogetaways.com",
  frontDesk: "(850) 654-1425",
};

export const OCCUPANCY = { maxPerUnit: 6, maxBothUnits: 12, adultsPerChildren: 3 };

// ── HOLIDAY WINDOWS — rolls forward automatically (no annual maintenance) ────
// Replaces v1's hardcoded HOLIDAY_DATES (chat.js 654-668). Computed for the
// current year and next year every request; injected as verified context.
function nthWeekdayOfMonth(year, month, weekday, n) {
  // month 0-11, weekday 0=Sun..6=Sat, n>=1
...
export function computeHolidayWindows(todayIso) {
  const out = [];
  const startYear = parseInt(todayIso.slice(0, 4), 10);
  for (const year of [startYear, startYear + 1]) {
    const memorial = lastWeekdayOfMonth(year, 4, 1);           // last Mon May
    const labor    = nthWeekdayOfMonth(year, 8, 1, 1);          // first Mon Sept
    const thanks   = nthWeekdayOfMonth(year, 10, 4, 4);         // 4th Thu Nov
    const july4    = new Date(Date.UTC(year, 6, 4));
    const xmas     = new Date(Date.UTC(year, 11, 25));
    const nye      = new Date(Date.UTC(year, 11, 31));
    out.push(
      { name: "Memorial Day weekend",  arrival: iso(shift(memorial, -3)), departure: iso(memorial) },
      { name: "4th of July weekend",   arrival: iso(shift(july4, -1)),    departure: iso(shift(july4, 2)) },
      { name: "Labor Day weekend",     arrival: iso(shift(labor, -3)),    departure: iso(labor) },
      { name: "Thanksgiving weekend",  arrival: iso(shift(thanks, -1)),   departure: iso(shift(thanks, 3)) },
      { name: "Christmas",             arrival: iso(shift(xmas, -1)),     departure: iso(shift(xmas, 2)) },
      { name: "New Year's",            arrival: iso(nye),                 departure: iso(shift(nye, 2)) },
    );
  }
  // Only future windows are useful — past ones caused v1's stale-holiday problem.
  return out.filter(h => h.departure >= todayIso);
}
