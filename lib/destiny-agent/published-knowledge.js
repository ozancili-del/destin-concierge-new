...
const IMPLICIT_MULTI_CHOICE_RE = /\b(?:restaurants|beaches|activities|attractions|shops|shopping|groceries|supermarkets|spas|airports|where (?:should|can|could|do) (?:we|i) eat|what (?:can|should|could) (?:we|i) do)\b/i;
...
const QUERY_TOPIC_RULES = Object.freeze([
  ...
  ["amenities", /\b(?:amenit(?:y|ies)|pools?|hot tubs?|saunas?|steam rooms?|gyms?|fitness|laundry|washers?|dryers?|parking|ev (?:chargers?|charging)|electric vehicles?|j1772)\b/i],
...
export function classifyPublishedKnowledgeRoute(query) {
  ...
  if (/\b(?:reservation|booking)\b.*\b(?:look up|find|change|modify|cancel|extend|confirmation|payment|card)\b/.test(value)) return "live";
...
export function rankPublishedKnowledge(bundle, { query = "", topics = [], limit = 8, requireMatch = false, excludeEntryIds = [] } = {}) {
  ...
  for (const topic of Array.isArray(bundle?.topics) ? bundle.topics : []) {
    ...
    for (const entry of Array.isArray(topic?.entries) ? topic.entries : []) {
      ...
      candidates.push({ ...rendered, score, matchedTerms, categoryRank, editorialPriority, distanceMinutes, categories: entry?.recommendation_categories || [] });
    }
  }
  ...
  for (const item of eligible) {
    ...
    const addsDiversity = !broadRecommendation || snippets.length < 1 || secondary.some(category => !usedSecondary.has(category));
  ...
  return { snippets: snippets.map(({ score, matchedTerms, categoryRank, editorialPriority, distanceMinutes, categories, ...item }) => item), urls };
}
