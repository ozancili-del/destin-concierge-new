...
export function validateProposal(value,{text,artifact}) {
  ...
  for(const a of value.actions){
    ...
    if(a.category!==null&&!categories.has(a.category))throw new Error('semantic_unknown_category');
...
export function createSemanticInterpreter({openai,model='gpt-5.6-sol',timeoutMs=20000}={}) {
  return async function interpret({text,history=[],state,pageContext,artifact,now,signal}) {
    ...
    const response=await openai.responses.create({model,store:false,instructions,input,text:{format:{type:'json_schema',name:'destiny_request',strict:true,schema:PROPOSAL_SCHEMA}},max_output_tokens:2500},{signal:abort});
    ...
    return {proposal:validateProposal(JSON.parse(output),{text,artifact}),evidence:{kind:'semantic_live',model,version:SEMANTIC_VERSION,inputHash:digest({instructions,input,schema:PROPOSAL_SCHEMA}),responseId:response.id||null}};
  };
}
