...
## August 24, 2026 SEO audit implementation backlog

Audit only; these items are recorded for deliberate implementation and measurement, not marked complete:

1. Assign one canonical owner page to each commercial keyword cluster: resort, vacation-rental collection, and the two unit pages.
2. Strengthen `/pelican-beach-resort-destin` as the primary landing page for “Pelican Beach Resort condos” and “Pelican Beach Resort rentals,” while keeping copy accurate to the family-owned inventory.
3. Reconcile the remaining legacy unit schema and breadcrumb identifiers so every signal names the clean canonical unit URLs.
4. Rebalance blog-to-commercial internal links: relevant guide traffic should reach the resort, vacation-rental collection, availability, and unit pages with natural intent-led anchors.
5. Narrow `/destin-vacation-rentals-by-owner` around owner-direct beachfront condo selection and remove overlap with the resort page.
6. Emit trustworthy sitemap `lastmod` values only when a page materially changes.
7. Complete route-specific Open Graph and Twitter metadata for the commercial landing pages.
8. Monitor Search Console and Bing canonical selection, indexing, query impressions, click-through rate, and cannibalization after recrawling.
9. Prioritize credible local/travel referral links and citations; do not manufacture a network of self-owned backlink sites.
10. Continue field-performance monitoring on both unit pages, especially mobile image delivery and third-party booking resources.
