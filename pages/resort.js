...
export default function Resort() {
  ...
  return <div className={styles.page}>
    ...
    <main>
      ...
      <form className={styles.availability} id="availability" method="post" action={`${liveSite}/properties`}><div><span>Live availability</span><strong>Search Units 707 and 1006</strong></div><label><span>Check in</span><input aria-label="Arrival date" name="ArrivalDate" type="date" required /></label><label><span>Check out</span><input aria-label="Departure date" name="DepartureDate" type="date" required /></label><label><span>Guests</span><select aria-label="Guests" name="Guests" defaultValue="2">{[1,2,3,4,5,6].map(n=><option key={n} value={n}>{n} {n===1?"guest":"guests"}</option>)}</select></label><input type="hidden" name="Page" value="1" /><input type="hidden" name="Sort" value="DailyRandom" /><SiteButton type="submit" variant="primary">Search dates</SiteButton></form>
