...
export const TOOL_DEFINITIONS = Object.freeze([
  ...
  },
  {
    ...
    function: {
      ...
      description: "Build a complete Aviasales flight search URL using a guest origin, booking dates, and passenger count. Code resolves supported cities and never guesses unclear text.",
      parameters: {
        type: "object",
        additionalProperties: false,
        properties: {
          origin_text: { ...nullableString, description: "Exact guest city or IATA wording, such as Chicago or ORD." },
          destination_iata: { type: ["string", "null"], enum: ["VPS", "PNS", "ECP", null] },
          infants: { type: ["integer", "null"], minimum: 0, maximum: 12 },
        },
        required: ["origin_text", "destination_iata", "infants"],
      },
    },
  },
  {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Build one complete TripShock activity URL for a validated activity category and optional stay dates.",
      parameters: {
        type: "object",
        additionalProperties: false,
        properties: {
        ...
        required: ["category", "date_text", "arrival", "departure"],
      },
    },
  },
  {
...
function scopeIsUsable(evidence, latestUser) {
  const message = String(latestUser || "");
  const lower = message.toLowerCase();
  const needle = String(evidence || "").toLowerCase();
  const index = lower.indexOf(needle);

  // Judge the evidence inside its own sentence/clause rather than a wide fixed
  // character window. A wide window incorrectly rejected valid text such as
  // "Can 707 fit us? Just my wife and me, no kids" because the earlier word
  // "fit" contaminated the current-party evidence.
  let context = lower;
  if (index >= 0) {
    ...
    const rightCandidates = [".", "?", "!", ";", "\n"]
      .map(mark => lower.indexOf(mark, index + needle.length))
      .filter(pos => pos >= 0);
    const rightBoundary = rightCandidates.length ? Math.min(...rightCandidates) : lower.length;
    context = lower.slice(leftBoundary + 1, rightBoundary);
  }
  ...
  if (/sleep(?:s)?|capacity|big enough|maximum|max guests?/i.test(context)) return false;
...
function tripVerificationResetIfChanged(state, nextBooking) {
  const keys = ["arrival", "departure", "adults", "children", "totalGuests"];
  const changed = keys.some(key => (state?.booking?.[key] ?? null) !== (nextBooking?.[key] ?? null));
...
export async function executeTool(name, args, context) {
  ...
  try {
    if (name === "remember_booking_details") {
      ...
      let patch = bookingStatePatch(input, "guest_message");
      const reset = tripVerificationResetIfChanged(state, patch.booking);
      if (reset) patch = { ...patch, ...reset };
    ...
    if (name === "build_booking_links") {
      ...
      const checkedAt = verified.availabilityCheckedAt ? new Date(verified.availabilityCheckedAt).getTime() : NaN;
      const ageMs = new Date(now).getTime() - checkedAt;
      const query = verified.availabilityQuery;
      const timestampValid = Number.isFinite(checkedAt) && ageMs >= -60_000 && ageMs <= 15 * 60 * 1000;
      if (!timestampValid || !query || !isIsoDate(query.arrival) || !isIsoDate(query.departure)) {
      ...
      const safeUrls = (verified.bookingUrls || []).filter(url => {
        try {
          const parsed = new URL(url);
          const unit = /unit-(707|1006)-orp/i.exec(parsed.pathname)?.[1] || null;
          if (!unit || verified.availabilityUnits?.[unit] !== true) return false;
          if (parsed.protocol !== "https:" || !["destincondogetaways.com", "www.destincondogetaways.com"].includes(parsed.hostname)) return false;
          return parsed.searchParams.get("or_arrival") === query.arrival
            && parsed.searchParams.get("or_departure") === query.departure
            && /^\d+$/.test(parsed.searchParams.get("or_adults") || "")
            && /^\d+$/.test(parsed.searchParams.get("or_children") || "");
        } catch (_) {
          return false;
        }
      });
      if (!safeUrls.length || safeUrls.length !== verified.bookingUrls.length) {
        return toolResult({ name, kind: "booking", ok: false, status: "invalid_persisted_verification", data: { checkedAt: verified.availabilityCheckedAt } });
      }
      return toolResult({ name, kind: "booking", data: { query, units: verified.availabilityUnits, resent: true }, urls: safeUrls, facts: verified.facts });
    }
    ...
    if (name === "build_flight_search") {
      ...
      const infants = normalizeNullableInteger(args.infants, 0, 12) ?? 0;
      if (!origin) return toolResult({ name, kind: "flight", ok: false, status: "needs_origin", data: { originText }, statePatch: { flight: { originIata: null, destinationIata: destination }, awaiting: mergeUnique(state.awaiting, ["origin_city"]) } });
      if (!state.booking.arrival || !state.booking.departure || state.booking.adults === null || state.booking.children === null) {
        const missing = [];
        if (!state.booking.arrival) missing.push("arrival");
        if (!state.booking.departure) missing.push("departure");
        if (state.booking.adults === null) missing.push("adults");
        if (state.booking.children === null) missing.push("children");
        return toolResult({ name, kind: "flight", ok: false, status: "needs_booking_details", data: { missing }, statePatch: { awaiting: mergeUnique(state.awaiting, missing), flight: { originIata: origin, destinationIata: destination } } });
      }
      const url = buildFlightLink(origin, state.booking.arrival, state.booking.departure, state.booking.adults, state.booking.children, infants, destination);
      const fact = `Flight search built from ${origin}${MULTI_AIRPORT_MAIN[origin] ? ` (${MULTI_AIRPORT_MAIN[origin]})` : ""} to ${destination} for ${state.booking.arrival} through ${state.booking.departure}.`;
      return toolResult({ name, kind: "flight", data: { origin, originLabel: MULTI_AIRPORT_MAIN[origin] || origin, destination, arrival: state.booking.arrival, departure: state.booking.departure, adults: state.booking.adults, children: state.booking.children, infants, url }, urls: [url], facts: [fact], statePatch: { flight: { originIata: origin, destinationIata: destination }, awaiting: state.awaiting.filter(x => x !== "origin_city"), verified: { flightUrls: [url], facts: [fact] } } });
    }
    ...
    if (name === "get_activity_options") {
      let dates = null;
      if (args.date_text && exactTextAppears(args.date_text, latestUser)) dates = extractDates(args.date_text, now) || (() => { const single = extractSingleDate(args.date_text, now); return single ? { arrival: single, departure: addIsoDays(single, 1) } : null; })();
      if (!dates && args.arrival && args.departure && String(latestUser).includes(args.arrival) && String(latestUser).includes(args.departure)) dates = { arrival: args.arrival, departure: args.departure };
      if (!dates && state.booking.arrival && state.booking.departure) dates = { arrival: state.booking.arrival, departure: state.booking.departure };
      const url = buildTripShockLink(args.category, dates);
      const fact = `Activity category ${args.category}${dates ? ` for ${dates.arrival} through ${dates.departure}` : ""}.`;
      return toolResult({ name, kind: "activity", data: { category: args.category, dates, url }, urls: [url], facts: [fact], statePatch: { verified: { activityUrls: [url], facts: [fact] } } });
    }
...
const SERIAL_TOOL_NAMES = new Set([
  ...
  "create_maintenance_alert", "capture_lead", "relay_owner_message", "request_owner_chat",
]);
const SINGLE_ACTION_PER_ROUND = new Set([
  "create_maintenance_alert", "capture_lead", "relay_owner_message", "request_owner_chat",
]);

function boundedMessageContent(value) {
  const text = String(value || "");
  return text.length <= MAX_HISTORY_MESSAGE_CHARS ? text : `${text.slice(0, MAX_HISTORY_MESSAGE_CHARS - 28)}\n[message truncated]`;
}
...
function conversationInput(messages) {
  const source = (Array.isArray(messages) ? messages : [])
    ...
    .map((message) => ({ role: message.role, content: boundedMessageContent(message.content) }));
  const kept = [];
  let total = 0;
  for (let index = source.length - 1; index >= 0; index -= 1) {
    const message = source[index];
    if (total + message.content.length > MAX_HISTORY_TOTAL_CHARS && kept.length) break;
    const remaining = Math.max(0, MAX_HISTORY_TOTAL_CHARS - total);
    kept.push({ ...message, content: message.content.slice(-remaining) });
    total += Math.min(message.content.length, remaining);
...
export async function runAgentTurn({
  ...
  const seenConsequentialActionsAcrossTurn = new Set(
    safety.toolResults.filter(result => SINGLE_ACTION_PER_ROUND.has(result?.name) && result?.ok).map(result => result.name)
  );
  ...
  for (let round = 0; round < maxToolRounds; round += 1) {
    ...
    const seenSingleActions = new Set();
    const entries = calls.map((call, index) => {
      const args = parseToolArguments(call);
      const entry = { call, args, index, result: null, execute: false };
      if (index >= maxToolCallsPerRound || totalExecutedToolCalls >= maxTotalToolCalls) {
        entry.result = toolResult({ name: call.name || "unknown", ok: false, status: "tool_call_limit_exceeded", error: "The server tool-call budget was exceeded." });
        return entry;
      }
      if (args.__parseError) {
        entry.result = toolResult({ name: call.name || "unknown", ok: false, status: "malformed_arguments", error: args.__parseError });
        return entry;
      }
      const signature = stableToolSignature(call.name, args);
      if (seenSignatures.has(signature)
        || (SINGLE_ACTION_PER_ROUND.has(call.name) && (seenSingleActions.has(call.name) || seenConsequentialActionsAcrossTurn.has(call.name)))) {
        entry.result = toolResult({ name: call.name || "unknown", ok: false, status: "duplicate_suppressed", error: "A duplicate or repeated consequential action was suppressed." });
        return entry;
      }
      seenSignatures.add(signature);
      if (SINGLE_ACTION_PER_ROUND.has(call.name)) {
        seenSingleActions.add(call.name);
        seenConsequentialActionsAcrossTurn.add(call.name);
      }
      entry.execute = true;
      totalExecutedToolCalls += 1;
      toolCallsDebug.push({ round: round + 1, callId: call.call_id, name: call.name, args });
      return entry;
    });

    const parallelPromises = new Map();
    for (const entry of entries) {
      if (!entry.execute || SERIAL_TOOL_NAMES.has(entry.call.name)) continue;
      parallelPromises.set(entry.index, withTimeout(executeTool(entry.call.name, entry.args, {
        services, state: workingState, latestUser, now, sessionId, guestBid, guestSig, pageSource, sawBanner, logger,
      }), toolTimeoutMs, `tool_${entry.call.name}`).catch((error) => toolResult({
        name: entry.call.name || "unknown", ok: false, status: "timeout_or_error", error: error.message,
      })));
    }

    for (const entry of entries) {
      if (entry.execute) {
        if (SERIAL_TOOL_NAMES.has(entry.call.name)) {
          entry.result = await withTimeout(executeTool(entry.call.name, entry.args, {
            services, state: workingState, latestUser, now, sessionId, guestBid, guestSig, pageSource, sawBanner, logger,
          }), toolTimeoutMs, `tool_${entry.call.name}`).catch((error) => toolResult({
            name: entry.call.name || "unknown", ok: false, status: "timeout_or_error", error: error.message,
          }));
        } else {
          entry.result = await parallelPromises.get(entry.index);
        }
      }
      ...
      input.push({
        ...
        output: JSON.stringify({ ok: result.ok, status: result.status, data: result.data, urls: result.urls, facts: result.facts, error: result.error }),
      });
    }
  }
