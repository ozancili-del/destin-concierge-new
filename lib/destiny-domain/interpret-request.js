...
export function interpretRouterInput(input) {
  ...
  const affirmative = /^(?:yes(?:,? please)?|yeah|yep|sure|okay|ok|please do|go ahead|do it)[,.! ]*$/.test(value.trim());
  const negative = /^(?:no(?:,? thanks|,? thank you)?|nope|do not|don't)[,.! ]*$/.test(value.trim());
  ...
  return Object.freeze({
    ...
    month,
    flags: Object.freeze({ acknowledgment, affirmative, negative, asksRepeat, asksRecommendations, protectedReservation, availability, liveFlight, liveVenueStatus, liveForecast, seasonalClimate, beachConditions, accommodation, livePrice, shopping, airports, genericFollowUp }),
