...
export function classifyPublishedKnowledgeRoute(query) {
  ...
  const restaurantKnowledge = inferPublishedKnowledgeTopics(query).includes("restaurants");
  ...
  if (/\b(?:open now|open today|hours?|wait time|closed|closure|close tonight|closes? (?:today|tonight)|reservation availability)\b/.test(value)) return restaurantKnowledge ? "knowledge" : "live";
...
export function rankPublishedKnowledge(bundle, { query = "", topics = [], limit = 8, requireMatch = false, excludeEntryIds = [] } = {}) {
  ...
  // Typed topics are a hard boundary for lists/recommendations. For factual
  // questions they are a routing preference, because one approved fact may
  // legitimately live under a unit/entity topic rather than its guide topic.
  const hardTopicBoundary = broadRecommendation || Boolean(recommendationCategory);
  ...
  for (const topic of Array.isArray(bundle?.topics) ? bundle.topics : []) {
    if (selectedTopics.size && hardTopicBoundary && !selectedTopics.has(topic?.topic_id)) continue;
    for (const entry of Array.isArray(topic?.entries) ? topic.entries : []) {
      ...
      let score = nameMatches * 12 + aliasTagMatches * 8 + questionMatches * 6 + evidenceMatches * 12 + voiceMatches * 2;
  ...
  // Stay inside the typed topic when it contains claim-level evidence. Expand
  // across entity topics only when the guide topic has no approved claim that
  // can satisfy the factual wording (for example, shared supplies stored under
  // each unit's record). This is a bounded cross-topic join, not a global free-for-all.
  const selectedTopicHasEvidence = !hardTopicBoundary && selectedTopics.size > 0
    && candidates.some(item => selectedTopics.has(item.topicId) && item.evidenceMatches > 0);
  if (selectedTopicHasEvidence) candidates = candidates.filter(item => selectedTopics.has(item.topicId));
