...
export function buildRouterDecision({ channel, text, sessionId, turnId, sequence = 1, context = {} } = {}) {
  const input = createRouterInput({ channel, originalText: text, sessionId, turnId, sequence, context });
  const plan = routeRequest(input);
  return Object.freeze({ ok: true, plan, summary: Object.freeze({ ...summarizeRouterPlan(plan), shadow: false }) });
}
