...
export class VoiceRecordingOutbox {
  ...
  save(id, file, blob) {
    ...
    this.saving.then(() => { this.onStatus("Saved on this device; syncing privately…"); this.flush(); }).catch(() => {
      this.saveFailure = true;
      this.onStatus("Local autosave failed. Keep this tab open and download the ZIP.");
    });
  ...
  async flush() {
    ...
    try {
      ...
      this.onStatus(this.saveFailure ? "Some local writes failed: keep this tab open and download the ZIP as backup." : "Recording chunks synced to private storage.");
    } catch { this.onStatus("Upload pending. Reopen this same URL on this device to retry."); }
    finally { this.busy = false; }
  }
  start() { if (this.closed) return; clearInterval(this.timer); this.flush(); this.timer = setInterval(() => this.flush(), 10000); }
