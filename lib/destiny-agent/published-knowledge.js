...
const STOP = new Set(["the", "and", "for", "with", "that", "this", "from", "your", "you", "are", "our", "have", "has", "what", "when", "where", "which", "does", "can", "will", "about", "into", "only", "guest", "guests", "destin", "near", "recommend", "some", "please", "tell", "generally", "like"]);
...
const QUERY_TOPIC_RULES = Object.freeze([
  ...
  ["everyday-essentials", /\b(?:starter kits?|supplies|toilet paper|paper towels?|soaps?|shampoo|conditioners?|dishwasher|trash bags?|coffee|grocer(?:y|ies)|supermarkets?|towels?)\b/i],
  ...
  ["transport", /\b(?:airports?|transport|rideshare|uber|lyft|rental cars?|without a car|drive time)\b/i],
  ...
  ["couples-and-quieter-stays", /\b(?:couple|romantic|romance|anniversary|quiet stay|snowbird)\b/i],
...
export function rankPublishedKnowledge(bundle, { query = "", topics = [], limit = 8, requireMatch = false } = {}) {
  ...
  if (selectedTopics.size === 1 && selectedTopics.has("restaurants")) {
    for (const locationWord of ["pelican", "beach", "resort", "nearby", "close"]) qTerms.delete(locationWord);
  }
  const candidates = [];
  ...
  const snippets = candidates
    .filter((item, index) => requireMatch ? item.matchedTerms === bestMatchCount && bestMatchCount > 0 : item.score > 0 || index < Math.min(2, candidates.length))
    .slice(0, cap);
