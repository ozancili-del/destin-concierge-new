...
export function createServices({ fetchImpl = globalThis.fetch, env = process.env, now = () => new Date(), logger = console } = {}) {
  ...
  async function sendOwnerChatInvite({ sessionId, guestMessage, inviteToken }) {
    try {
      ...
      const ownerChatBaseUrl = deploymentHost
        ...
        : "https://www.destincondogetaways.com";
  ...
  async function fetchCalendarAlternatives(arrival, departure) {
    try {
      const response = await fetchWithTimeout(fetchImpl, `https://www.destincondogetaways.com/api/calendar?arrival=${encodeURIComponent(arrival)}&departure=${encodeURIComponent(departure)}`, {}, 9000);
      if (!response.ok) return null;
  ...
  async function fetchPriceDrops(arrival, departure) {
    try {
      const response = await fetchWithTimeout(fetchImpl, `https://www.destincondogetaways.com/api/price-drops?arrival=${encodeURIComponent(arrival)}&departure=${encodeURIComponent(departure)}`, {}, 5000);
  ...
  async function runAdminPriceSnapshot() {
    try {
      const snapRes = await fetchWithTimeout(fetchImpl, "https://www.destincondogetaways.com/api/price-snapshot", {
        method: "GET",
