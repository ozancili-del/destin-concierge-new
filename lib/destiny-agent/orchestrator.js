...
export const TOOL_DEFINITIONS = Object.freeze([
              ...
              properties: {
                ...
                required_tool: {
                  ...
                  enum: [
                    ...
                    "get_destin_weather", "get_beach_conditions", "get_beach_deals", "get_local_guide", "search_current_events", "get_activity_options",
  ...
  },
  {
    ...
    function: {
      ...
      description: "Read the currently published active price reductions from the Destin Condo Getaways Beach Deals page. Use for current deals, specials, price drops, flexible reduced-price dates, or the closest reduced dates. This does not confirm availability; call check_availability separately before presenting a stay as bookable. Never suggest or mention Make an Offer.",
      parameters: {
        type: "object",
        additionalProperties: false,
        properties: {
          date_text: { ...nullableString, description: "Exact guest date wording from the latest message, or null to reuse confirmed booking dates." },
        ...
        required: ["date_text", "arrival", "departure", "preferred_unit"],
      },
    },
  },
  {
  ...
  try {
    ...
    if (name === "get_beach_deals") {
      ...
      const facts = [
        ...
        "Include the Beach Deals page so the guest can browse all currently published reductions. Never mention, suggest, or link to Make an Offer.",
      ];
      return toolResult({
        name,
        kind: "beach_deals",
        ...
        data: { query: dates, unit, matchType: result?.matchType || null, deals, checkedAt: result?.checkedAt || null },
        urls,
        facts,
    ...
    if (name === "search_current_events") {
      ...
      const discoveryUrls = category === "events" ? [] : [bandsintownUrl];
      ...
      try {
        ...
        const fallbackFact = `The live schedule lookup did not complete. Do not ask the guest to repeat the request. Provide the included ${requestedArea} discovery page and curated guide so the guest can immediately see current options, and say that dates and times should be cross-checked.`;
        ...
        return toolResult({
          ...
          urls,
          facts: [fallbackFact],
          statePatch: { mode: "local_info", verified: { blogUrls: urls, facts: [fallbackFact] } },
        });
      }
