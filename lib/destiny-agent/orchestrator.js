  ...
  try {
    ...
    if (name === "search_current_events") {
      ...
      const searchAttempts = await Promise.allSettled([
        callResponses(openai, makeSearchRequest("Official Destin, Miramar Beach, and Fort Walton Beach venue and organizer calendars."), 26000, "current_events_search_venues"),
        callResponses(openai, makeSearchRequest("Primary ticketing listings and nearby official calendars; fill gaps without duplicating unsupported claims."), 26000, "current_events_search_tickets"),
      ]);
