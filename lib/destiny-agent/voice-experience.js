...
export function isDirectedVoiceUtterance(value, { duringPlayback = false } = {}) {
  const normalized = normalizeVoiceUtterance(value);
  const words = normalized ? normalized.split(" ") : [];
  if (words.length < 3) return false;
  if (!duringPlayback) return true;
  return /^(destiny\b|can\b|could\b|would\b|will\b|do\b|does\b|did\b|is\b|are\b|what\b|when\b|where\b|why\b|how\b|tell\b|show\b|check\b|find\b|book\b|recommend\b|wait\b|hold\b)/.test(normalized)
    || /\b(?:can|could|would|will|do|does|did) you\b/.test(normalized)
    || /\b(weather|restaurants?|availability|booking|condo|units?|amenit(?:y|ies)|pools?|sauna|gym|questions?)\b/.test(normalized);
}

export function isLikelyAssistantEcho(value, assistantValue) {
  const heard = normalizeVoiceUtterance(value);
  const spoken = normalizeVoiceUtterance(assistantValue);
  const heardWords = heard ? heard.split(" ") : [];
  if (heardWords.length < 3 || !spoken) return false;
  if (spoken.includes(heard)) return true;
  const spokenWords = new Set(spoken.split(" "));
  const matchingWords = heardWords.filter(word => spokenWords.has(word)).length;
  return matchingWords / heardWords.length >= 0.85;
}

export function inferExpectedVoiceReply(value) {
  const raw = String(value || "").trim();
  const normalized = normalizeVoiceUtterance(raw);
  if (!raw.includes("?")) return "";
  if (/\b(how many|adults?|children?|guests?|party size)\b/.test(normalized)) return "party";
  if (/\b(check-?in|check-?out|dates?|year|when)\b/.test(normalized)) return "date";
  if (/\b(707|1006|which (?:one|unit)|choose|prefer)\b/.test(normalized)) return "choice";
  if (/\b(english|turkish|language|continue in|switch to)\b/.test(normalized)) return "language";
  if (/^(?:do|does|did|is|are|would|will|can|could|have|has)\b/.test(normalized)) return "confirmation";
  return "";
}

export function isExpectedVoiceReply(value, expectedKind) {
  const normalized = normalizeVoiceUtterance(value);
  if (!normalized || normalized.split(" ").length > 5) return false;
  if (expectedKind === "party") return /\b(?:zero|one|two|three|four|five|six|seven|eight|nine|ten|\d+|adult|child|kid|guest)\b/.test(normalized);
  if (expectedKind === "date") return /\b(?:january|february|march|april|may|june|july|august|september|october|november|december|today|tomorrow|tonight|week|\d{1,4})\b/.test(normalized);
  if (expectedKind === "choice") return /\b(?:707|1006|unit|first|second|either|both)\b/.test(normalized);
  if (expectedKind === "language") return /^[\p{L}-]+(?:\s+[\p{L}-]+){0,2}$/u.test(normalized);
  if (expectedKind === "confirmation") return /^(?:yes|yeah|yep|no|nope|sure|okay|ok|please|correct|right)$/.test(normalized);
  return false;
}
...
${immediateFactText}
2. Approved stable knowledge: use get_approved_knowledge immediately for facts not listed above, including condo and resort details, policies, arrival logistics, parking and EV-charger details, accessibility guidance, typical monthly or seasonal climate, and established local or restaurant recommendations. These are not live searches. Call this tool silently—do not say “let me check,” “let me pull,” or give a progress update. For a follow-up such as “Where are they?” or “What other ones?”, include its subject from the conversation in the query. For a broad recommendation request, offer three distinct approved choices by default unless the guest requests a different number or the tool returns fewer; name every candidate returned and give one natural differentiator for each. Never collapse multiple returned candidates into a single option and never invent filler.
3. Fresh checks: availability, near-term weather forecasts, current beach conditions, current restaurant hours or closures, current events, changing prices, and schedules require a live tool. For check_live_availability, call the tool silently because it is usually quick. Before ask_destiny_brain, say one short, relevant acknowledgement that tells the guest what live information you are checking. Keep it to one sentence, never promise how long it will take, and then call the tool immediately. This is a conversational bridge while verified work runs, not entertainment or a result. Do not repeat the acknowledgement while the same lookup is pending. If the guest gives a short presence check such as “Hello?”, “Are you there?”, or “Still checking?” while that lookup is pending, reassure them about the same lookup without starting another tool call.
4. Protected checks: reservation details, door codes, maintenance actions, or anything requiring authorization must use ask_destiny_brain. Never reveal protected information unless the returned result authorizes it.
...
Use ask_destiny_brain for live or changing information, pricing, complex booking work, protected information, guest support, and existing reservations. Use get_approved_knowledge instead for stable policies and local recommendations. Preserve exact dates and party wording in a brain-tool query, including explicit zero values. Never say “including zero” to a guest. Ask naturally: “How many adults and children are traveling?” Treat “no children” as zero internally. If a guest says they cannot use the internet or cannot complete online checkout, use ask_destiny_brain for the approved human-assisted booking route; do not claim a reservation, hold, or payment is being completed. Never mention internal vendors or booking-platform names to a guest. After a tool returns, summarize the useful answer naturally instead of reading it word-for-word. For greetings, thanks, and casual turns, answer directly. If a tool result contains a link, say briefly that you added it below, but never speak the URL. If the guest interrupts, stop immediately and listen.`;
