...
function bookingInputFromArgs(args, context, { allowZeroChildBaseline = false } = {}) {
  ...
  let dates = dateAmbiguous ? null : groundedStructuredDateRange({
  ...
  if (!dateAmbiguous && !dates && dateText && exactTextAppears(dateText, latestUser)) {
  ...
  const assumedChildrenZero = allowZeroChildBaseline && explicitChildren === null && state.booking.children === null && adults !== null && (!statedChildCategory || explicitAdultOnlyTravelingParty);
  ...
  return { dates, adults, children, totalGuests, preferredUnit, bedroomsRequested, explicitAdults, explicitChildren, explicitTotalGuests, assumedChildrenZero, dateText, dateAmbiguous };
}
...
const COMPLETION_TASKS = Object.freeze([
  { id: "availability", tools: ["check_availability", "find_open_windows", "build_booking_links"], pattern: /\b(?:707|1006|condos?|units?|stay)\b.*\b(?:avail(?:able|ability)?|open|free|fit|book|reserve|check)\b|\b(?:avail(?:able|ability)?|book|reserve|check|fit)\b.*\b(?:707|1006|condos?|units?|stay)\b|\bcan we stay\b/i },
  { id: "weather", tools: ["get_destin_weather"], pattern: /\b(?:weather|forecast|rain|raining|temperature|how hot|water conditions)\b/i },
  { id: "activities", tools: ["get_activity_options"], pattern: /\b(?:dolphin|parasail|parasailing|jet ski|jetski|pontoon|sunset cruise|boat tour|fishing charter|beach photographer)\b/i },
  { id: "flights", tools: ["build_flight_search"], pattern: /\b(?:flight|flights|fly|flying|airfare|airport)\b/i },
  { id: "local_guide", tools: ["get_local_guide", "get_business_knowledge"], pattern: /\b(?:restaurants?|where (?:should|can) we eat|events?|rental car|three-day itinerary|3-day itinerary)\b/i },
]);

function requestedCompletionTasks(latestUser, state) {
  const tasks = COMPLETION_TASKS.filter(task => task.pattern.test(String(latestUser || "")));
  const clearStayExtension = Boolean(state?.booking?.arrival && state?.booking?.departure)
    && /\bstay\s+(?:one|1|two|2)\s+(?:more|extra)\s+days?\b/i.test(String(latestUser || ""));
  if (clearStayExtension && !tasks.some(task => task.id === "availability")) tasks.push(COMPLETION_TASKS.find(task => task.id === "availability"));
  return tasks.length >= 2 || clearStayExtension ? tasks : [];
}

function incompleteRequestedTasks(tasks, toolResults) {
  const attempted = new Set((toolResults || []).map(result => result?.name).filter(Boolean));
  return tasks.filter(task => !task.tools.some(tool => attempted.has(tool)));
}

function isItineraryPlannerRequest(text) {
  return /\b(?:itinerary|trip planner|plan my trip|plan my vacation|day[- ]by[- ]day plan|three-day destin plan|3-day destin plan)\b/i.test(String(text || ""));
}

function isBeachPhotographerRequest(text) {
  return /\b(?:beach photographer|beach photography|family (?:beach )?(?:photos|pictures)|photo session on the beach)\b/i.test(String(text || ""));
}
...
export async function runAgentTurn({
  ...
  // Preserve the owner's dedicated zero-friction integrations. These requests
  // should never be replaced with a generic AI-generated itinerary or promises
  // to contact/book a photographer.
  if (isItineraryPlannerRequest(latestUser)) {
    const result = await executeTool("get_local_guide", { topic: "itinerary" }, { services, state: workingState, messages, latestUser, now, sessionId, guestBid, guestSig, pageSource, sawBanner, logger });
    toolResults.push(result);
    workingState = mergeToolPatch(workingState, result.statePatch);
    const reply = `Oh, you have to try our Destin Vacation Itinerary Planner — choose your dates, group, pace, food preferences, and interests, and it builds the full day-by-day plan for you:\n${STATIC_URLS.tripPlanner}`;
    return { reply, state: normalizeState(workingState), toolResults, detectedIntent: "LOCAL_INFO", debug: { agentic: true, safetyIntercept: "itinerary_planner", toolCalls: [{ round: 0, name: "get_local_guide", args: { topic: "itinerary" } }], toolRounds: 0, validation: { ok: true, violations: [] } } };
  }

  if (isBeachPhotographerRequest(latestUser)) {
    const result = await executeTool("get_activity_options", { category: "photographer", start_date: null, end_date: null }, { services, state: workingState, messages, latestUser, now, sessionId, guestBid, guestSig, pageSource, sawBanner, logger });
    toolResults.push(result);
    workingState = mergeToolPatch(workingState, result.statePatch);
    const reply = `Here is the TripShock beach-photographer browsing page for current options and provider details. This link does not confirm live pricing or availability:\n${result.urls[0]}`;
    return { reply, state: normalizeState(workingState), toolResults, detectedIntent: "LOCAL_INFO", debug: { agentic: true, safetyIntercept: "tripshock_photographer", toolCalls: [{ round: 0, name: "get_activity_options", args: { category: "photographer" } }], toolRounds: 0, validation: { ok: true, violations: [] } } };
  }
  ...
  const requestedTasks = requestedCompletionTasks(latestUser, workingState);
  ...
  for (let round = 0; round < maxToolRounds; round += 1) {
    ...
    if (!calls.length) {
      const missingTasks = incompleteRequestedTasks(requestedTasks, toolResults);
      if (missingTasks.length && round + 1 < maxToolRounds && executedToolCalls < maxTotalToolCalls) {
        input.push(...(response.output || []));
        input.push({
          role: "developer",
          content: `COMPLETION CHECK: You have not yet attempted these distinct guest requests: ${missingTasks.map(task => task.id).join(", ")}. Use the appropriate tools now, within the remaining budget. Complete every item; if a tool fails, preserve that result and explain the failure honestly in the final reply.`,
        });
    ...
    const roundResults = await Promise.all(calls.map(async (call) => {
      ...
      if (roundExecutedToolCalls >= maxToolCallsPerRound || executedToolCalls >= maxTotalToolCalls) {
  ...
  return {
    ...
    debug: {
      ...
      responseDiagnostics,
      completion: {
        requested: requestedTasks.map(task => task.id),
        attempted: requestedTasks.filter(task => !incompleteRequestedTasks([task], toolResults).length).map(task => task.id),
