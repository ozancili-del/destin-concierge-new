...
export function extractDates(input, now = new Date()) {
  ...
  // Preserve both explicit years in full US numeric ranges. Without this,
  // "07/07/2027-07/14/2027" falls through to single-date parsing and is
  // incorrectly treated as a one-night July 7-8 range.
  const fullNumericRange = t.match(/\b(\d{1,2})\/(\d{1,2})\/(20\d{2})\s*(?:[-–]|to|until|through|thru)\s*(\d{1,2})\/(\d{1,2})\/(20\d{2})\b/i);
  if (fullNumericRange) {
    const result = {
      arrival: `${fullNumericRange[3]}-${fullNumericRange[1].padStart(2, "0")}-${fullNumericRange[2].padStart(2, "0")}`,
      departure: `${fullNumericRange[6]}-${fullNumericRange[4].padStart(2, "0")}-${fullNumericRange[5].padStart(2, "0")}`,
    };
    if (isIsoDate(result.arrival) && isIsoDate(result.departure)) return result;
  }
